...
public class PlayerMovement : MonoBehaviour
{
    ...
    void HandleXMovement()
    {
        ...
        {
            float increment = xInput * Acceleration * Time.deltaTime;
            float targetSpeed = Mathf.Clamp(body.velocity.x + increment, -MaxSpeed, MaxSpeed);
            float velocityChange = targetSpeed - body.velocity.x;
            body.AddForce(new Vector2(velocityChange * body.mass, 0), ForceMode2D.Impulse);
    ...
    void PerformJump()
    {
        float velocityChange = JumpPower - body.velocity.y;
        body.AddForce(new Vector2(0, velocityChange * body.mass), ForceMode2D.Impulse);
        currentVerticalSpeed = JumpPower;
    }
    ...
    {
        float gravityMultiplier = grounded ? 1f : (apexPoint > 0.5f ? 1f : JumpEndEarlyGravityModifier);
        float gravityForce = -FallAcceleration * gravityMultiplier * Time.deltaTime;
        body.AddForce(new Vector2(0, gravityForce * body.mass), ForceMode2D.Force);
    }
    ...
    void ClampFallSpeed()
    {
        ...
        {
            float velocityChange = -MaxFallSpeed - body.velocity.y;
            body.AddForce(new Vector2(0, velocityChange * body.mass), ForceMode2D.Impulse);
        }
    }
    ...
    void ApplyFriction()
    {
        if (grounded && Mathf.Abs(xInput) < HorizontalDeadZoneThreshold)
        {
            float velocityChange = -body.velocity.x * GroundDeceleration * Time.fixedDeltaTime;
            body.AddForce(new Vector2(velocityChange * body.mass, 0), ForceMode2D.Force);
        }
    }
}
